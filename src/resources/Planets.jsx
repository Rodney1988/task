...
import { DynamicHeaders, DynamicRows } from "./PlanetColumns";
import { getPlanets } from "../Api";
import { StyledDiv } from "../styles";
import { AuthorizationWrapper, ReturnButton } from "../helpers";

// This renders a table of all planets
export const Planets = () => {
  // Ensure the state has at least an empty object
  const [rows, setRows] = useState([]);
  const { data, isLoading, isError } = useQuery(["getPlanets"], () =>
    getPlanets()
  );
  ...
  if (isError) {
    return <Alert severity="error">Error fetching Planets Data</Alert>;
  }
    ...
    <AuthorizationWrapper>
      <StyledDivWrapper>
        ...
        <Paper>
          ...
          <Typography variant="h7">{data.count} planets in total</Typography>
          <SearchBar
