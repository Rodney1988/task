...
import { DynamicHeaders, DynamicRows } from "./StarshipColumns";
...
// This renders a table of all starships
export const Starships = () => {
  // Ensure the state has at least an empty object
  const [rows, setRows] = useState([]);
  const { data, isLoading, isError } = useQuery(["getStarships"], () =>
    getStarships()
  );
  ...
  if (isError) {
    return <Alert severity="error">Error fetching Starships Data</Alert>;
  }
    ...
    <AuthorizationWrapper>
      <StyledDivWrapper>
        ...
        <Paper>
          ...
          <Typography variant="h7">{data.count} starships in total</Typography>
          <SearchBar
